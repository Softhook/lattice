#!/usr/bin/env node
/**
 * **Look at the twenty seconds, since I cannot hear them.**
 *
 * ```bash
 * node tools/trailer/score/analyze.mjs
 * ```
 *
 ...
 * you that the montage is a solid block of mid-range with no gaps in it, that the hole at 0:10 is
 * actually a hole, and that the last chord decays instead of stopping.
 *
...
import { CUES, DURATION_SEC } from './score.mjs';
...
function picture(channels, rate, file) {
  ...
  const rules = [
    [CUES.pulseIn, [90, 90, 90]],
    [CUES.montage, [255, 255, 255]],
    [CUES.cut, [255, 90, 90]],
    [CUES.holeEnd, [90, 90, 90]],
    ...CUES.cycles.map((time) => [time, [120, 255, 140]]),
    ...CUES.plays.map((time) => [time, [60, 150, 80]]),
    [CUES.lift, [255, 220, 90]],
    [CUES.land, [255, 255, 255]],
  ];
...
function main() {
  ...
  const sections = [
    ['establish   0.0–3.0', 0, 3],
    ['montage     3.0–10.0', 3, 10],
    ['  first bar  3.0–4.0', 3, 4],
    ['  last bar   8.0–10.0', 8, 10],
    ['the hole   10.3–11.0', 10.3, 11],
    ['typing 1   11.0–12.25', 11, 12.25],
    ['cycles     11.0–17.0', 11, 17],
    ['lift       17.0–18.0', 17, 18],
    ['end card   18.0–20.0', 18, 20],
  ...
  for (let second = 0; second < DURATION_SEC; second += 1) {
    ...
    const bars = Math.max(0, Math.round((level + 46) / 1.2));
  ...
  // Every landmark the edit will cut on, plus the montage's beat grid.
  // The sixteenth-note grid the sequencer plays on, both windows, plus everything struck by hand.
  const grid = [
    ...Array.from({ length: 65 }, (unused, index) => 2 + index * 0.125),
    ...Array.from({ length: 49 }, (unused, index) => 12 + index * 0.125),
    ...[
      [11, 0.125, 10],
      [13.5, 0.0625, 14],
      [15.5, 0.03125, 16],
    ].flatMap(([from, gap, count]) => Array.from({ length: count }, (unused, index) => from + index * gap)),
    ...CUES.cycles,
    ...CUES.plays,
    0,
    0.65,
