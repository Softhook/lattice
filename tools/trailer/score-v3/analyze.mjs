 ...
 *
 * ```bash
 * node tools/trailer/score-v3/analyze.mjs
 * ```
 *
 ...
 * | the button 0:08.5, the re-strike 0:27.9 | blue |
...
function picture(channels, rate, file) {
  ...
  const rules = [
    ...
    [CUES.restrike, [120, 200, 255]],
    [CUES.button, [90, 150, 200]],
...
function main() {
  ...
  const sections = [
    ...
    ['  the ring   21.73-22.20', CUES.flash, CUES.returnAt],
    ['3 embers     22.20-24.10', CUES.returnAt, CUES.firstLight],
  ...
  const grid = [
    ...
    ...[
      ...
      CUES.firstLight, CUES.endCard, CUES.restrike, CUES.button,
    ],
  ];
  ...
  const blind = measure(channels, CUES.drop[0], CUES.drop[1], sampleRate);
  ...
  const blindTail = measure(channels, CUES.drop[1] - 0.1, CUES.drop[1], sampleRate);
  console.log(
    `  the drop ${CUES.drop[0].toFixed(2)}-${CUES.drop[1].toFixed(2)} s: rms ${show(db(blind.rms))} over the whole window ` +
      `(${(db(blind.rms) - db(before.rms)).toFixed(1)} dB under the half second before it), ` +
